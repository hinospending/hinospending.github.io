/*
 * Icons for COFOG taxonomy
 *
 */
var Taxes = Taxes || {};

Taxes.baseKoujo = 330000; // 住民税基礎控除
Taxes.huyoKoujo = 330000; // 一人分の扶養控除
Taxes.taxRate = 0.06; // 住民税率

var OpenSpending = OpenSpending || {};

OpenSpending.identifier = 'hino_2014_budget';
OpenSpending.year = '2014';

OpenSpending.Styles = OpenSpending.Styles || {};

OpenSpending.Styles.Cofog = {
    /*議会費*/
  '1': { icon: 'icons/government.svg', color: '#C75746', bcolor: '#935B3B' },
/*議会費*/
  '101': { icon: 'icons/government.svg', color: '#C75746', bcolor: '#935B3B' },
/*総務費*/
  '2': { icon: 'icons/publicaffairs.svg', color: '#C75746', bcolor: '#935B3B' },
    /*総務管理費*/
  '201': { icon: 'icons/publicaffairs.svg', color: '#C75746', bcolor: '#0AB971' },
    /*徴税費*/
  '202': { icon: 'icons/money.svg', color: '#C75746', bcolor: '#0AB971' },
    /*戸籍住民基本台帳費*/
  '203': { icon: 'icons/publicaffairs.svg', color: '#C75746', bcolor: '#0AB971' },
    /*選挙費*/
  '204': { icon: 'icons/vote.svg', color: '#C75746', bcolor: '#4E6D00' },
/*統計調査費*/
  '205': { icon: 'icons/ekonomija.svg', color: '#C75746', bcolor: '#4E6D00' },
/*監査委員費*/
  '206': { icon: 'icons/publicaffairs.svg', color: '#C75746', bcolor: '#4E6D00' },
/*民生費*/
  '3': { icon: 'icons/civilian-action.svg', color: '#C75746', bcolor: '#4E6D00' },
  /*社会福祉費*/
  '301': { icon: 'icons/helping-others.svg', color: '#C75746', bcolor: '#D33673' },
  /*児童福祉費*/
  '302': { icon: 'icons/schools.svg', color: '#C75746', bcolor: '#D33673' },
  /*生活保護費*/
  '303': { icon: 'icons/unemployment.svg', color: '#C75746', bcolor: '#D33673' },
  /*災害救助費*/
  '304': { icon: 'icons/restoration.svg', color: '#C75746', bcolor: '#D33673' },
/*衛生費*/
  '4': { icon: 'icons/health.svg', color: '#C75746', bcolor: '#2A3A03' },
/*保健衛生費*/
  '401': { icon: 'icons/health.svg', color: '#C75746', bcolor: '#2A3A03'  },
/*清掃費*/
  '402': { icon: 'icons/garbage.svg', color: '#C75746', bcolor: '#2A3A03'  },
/*病院費*/
  '403': { icon: 'icons/hospital.svg', color: '#C75746', bcolor: '#EC2406' },
/*労働費*/
  '5': { icon: 'icons/labour.svg', color: '#C75746', bcolor: '#EC2406' },
/*労働諸費*/
'501': { icon: 'icons/labour.svg', color: '#C75746', bcolor: '#EC2406' },
/*農業費*/
  '6': { icon: 'icons/farms.svg', color: '#C75746', bcolor: '#938626' },
/*農業費*/
  '601': { icon: 'icons/farms.svg', color: '#C75746', bcolor: '#938626' },
/*商工費*/
  '7': { icon: 'icons/economic-aid.svg', color: '#C75746', bcolor: '#938626' },
/*商工費*/
  '701': { icon: 'icons/economic-aid.svg', color: '#C75746', bcolor: '#938626' },
/*土木費*/
  '8': { icon: 'icons/construction.svg', color: '#C75746', bcolor: '#C75746' },
/*土木管理費*/
  '801': { icon: 'icons/construction.svg', color: '#C75746', bcolor: '#C75746' },
/*道路橋梁費*/
  '802': { icon: 'icons/our-streets.svg', color: '#C75746', bcolor: '#C75746' },
/*河川費*/
  '803': { icon: 'icons/c_watersup.svg', color: '#C75746', bcolor: '#D33673' },
/*都市計画費*/
  '804': { icon: 'icons/machi.svg', color: '#C75746', bcolor: '#D33673' },
/*住宅費*/
  '805': { icon: 'icons/housing.svg', color: '#C75746', bcolor: '#D33673' },
/*消防費*/
  '9': { icon: 'icons/order-safety.svg', color: '#C75746', bcolor: '#790586' },
/*消防費*/
  '901': { icon: 'icons/order-safety.svg', color: '#C75746', bcolor: '#790586' },
/*教育費*/
  '10': { icon: 'icons/schools.svg', color: '#C75746', bcolor: '#790586' },
/*教育総務費*/
 '191': { icon: 'icons/admin.svg', color: '#C75746', bcolor: '#790586' },
/*小学校費*/
 '192': { icon: 'icons/primary.svg', color: '#C75746', bcolor: '#790586' },
/*中学校費*/
 '193': { icon: 'icons/secondary-lower.svg', color: '#C75746', bcolor: '#790586' },
/*幼稚園費*/
 '194': { icon: 'icons/pre-school.svg', color: '#C75746', bcolor: '#790586' },
/*社会教育費*/
 '195': { icon: 'icons/misc-services.svg', color: '#C75746', bcolor: '#790586' },
/*体育費*/
 '196': { icon: 'icons/sports.svg', color: '#C75746', bcolor: '#790586' },
/*公債費*/
 '11': { icon: 'icons/money.svg', color: '#C75746', bcolor: '#790586' },
/*公債費*/
 '111': { icon: 'icons/money.svg', color: '#C75746', bcolor: '#790586' },
/*公営企業費*/
    '12': { icon: 'icons/traffic-watersup.svg', color: '#C75746', bcolor: '#790586' },
/*公営企業費*/
    '121': { icon: 'icons/traffic-watersup.svg', color: '#C75746', bcolor: '#790586' },
/*予備費*/
 '13': { icon: 'icons/pig.svg', color: '#C75746', bcolor: '#790586' },
/*予備費*/
 '131': { icon: 'icons/pig.svg', color: '#C75746', bcolor: '#790586' }
};



